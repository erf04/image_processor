...
BMPImage * bmp_sepia(BMPImage* image){
    ...
    for (int i=0;i< image->height * image->width;i++){
        RGB pixel=image->data[i];
        r=((0.393*pixel.r) + (0.769* pixel.g) + (0.189*pixel.b));
        g=((0.349*pixel.r) + ( 0.686* pixel.g) + (0.168*pixel.b));
        b=((0.272*pixel.r) + (0.534* pixel.g) + (0.131*pixel.b));
        if (r>255)
            r=255;
        if (g>255)
            g=255;
